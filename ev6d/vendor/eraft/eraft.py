...
from .padding import ImagePadder

def autocast(enabled=False):
    # PyTorch >= 2.5 API; mixed precision remains disabled as in upstream.
...
class ERAFT(nn.Module):
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, upsample=True):
        ...
        if image1.shape != image2.shape or image1.ndim != 4:
            raise ValueError('Expected equal [B,C,H,W] input tensors')
        if iters < 1:
            raise ValueError('iters must be positive')
        # Padding is per call: switching image geometry must not retain old state.
        self.image_padder = ImagePadder(min_size=32)
